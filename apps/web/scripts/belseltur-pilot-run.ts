...
// (https://belseltur.com), requested by the Owner explicitly as a "first
// real test": research + strategy + media plan must use real tool/model
// calls, but NOTHING that creates or configures a real external account
// is allowed to run this time (no Google Ads/Meta Ads campaign creation,
// no analytics/GTM provisioning). Google Ads and Meta Ads credentials
// *are* present in .env.local, which means the normal PPC node in
// lib/orchestrator.ts would actually create paused campaigns for real if
// invoked through the standard /api/projects -> approve flow. This script
// exists to prove out Research -> CEO -> PM -> Media Buyer -> PPC
// (reasoning only) -> Copywriter -> QA -> a hand-assembled report while
// keeping every ad-platform tool call BLOCKED and logged instead of executed.
//
// Usage:
//   node --env-file=apps/web/.env.local apps/web/scripts/belseltur-pilot-run.ts
...
const PROJECT_ID = asProjectId("belseltur-pilot-2026-08-27");

const SITE_URL = "https://belseltur.com";

interface LogEntry {
  readonly seq: number;
  readonly kind: "agent-call" | "tool-call";
  readonly from: string; // who initiated
  readonly to: string; // agent role or tool id
  readonly asked: unknown; // task description / args
  readonly answered: unknown; // decision summary / result
...
// ---------------------------------------------------------------------
// Singletons (mirrors apps/web/lib/singletons.ts, standalone here so this
// script doesn't depend on Next.js's request-scoped globalThis stash).
// ---------------------------------------------------------------------
...
// Tools that are safe to run for real this pass: read-only data
// collection, no external account/state is created or changed.
const SAFE_REAL_TOOLS = new Set(["site-reader", "web-search"]);

// Everything else — in particular google-ads / meta-ads / vk-ads /
// yandex-direct / *-analytics / datalens / deployment-tool — is BLOCKED:
// the Owner explicitly said not to create or configure any account this
// run. The call is logged as if it had been attempted, so the report can
// show exactly what the PPC Agent *tried* to do.
const guardedInvoker: ToolInvoker = async (toolId, args) => {
  if (SAFE_REAL_TOOLS.has(toolId)) {
    try {
      ...
      // Real call attempted and genuinely failed (e.g. an expired/invalid
      // provider key) — surfaced honestly in the comm log rather than
      // silently degrading, but does not abort the whole research task:
      // the agent still gets the other real source (site content).
...
async function main() {
  ...
  const { agentInput: researchInput } = prepareResearchInvocation({
    ...
    taskDescription: "Изучить сайт потенциального клиента и рынок (туризм/экскурсии, Беларусь), собрать факты для стратегии продвижения через Google Ads и Meta Ads.",
    siteUrl: SITE_URL,
    // No topic guessed here on purpose (see research-agent.ts's 2026-08-27
    // fix) — this hint only adds the channel focus; the actual search
    // query is built by the agent from the real, just-fetched site content.
    searchQuery: "фокус на продвижении через Google Ads и Meta Ads",
    complexity: "standard",
  ...
  const { agentInput: ceoInput } = prepareCeoInvocation({
    ...
    taskDescription: "Определить стратегию продвижения потенциального клиента на основе реального ресёрча.",
    siteUrl: SITE_URL,
    businessDescription: `Ресёрч Research Agent:\n${findingsSummary}\n\nФакты:\n- ${findingsFacts}`,
    product: "Туристические/экскурсионные услуги (согласно сайту)",
    marketingTask: "Стратегия продвижения через Google Ads и Meta Ads для нового клиента, с предварительным медиапланом и прогнозом конверсий",
    complexity: "standard",
    invokeTool: guardedInvoker,
  });
  ...
  const { agentInput: pmInput } = preparePmInvocation({
    ...
    taskDescription: "Построить план исполнения для стратегии продвижения (Google Ads + Meta Ads).",
  ...
  console.log("\n=== 4/8 Media Buyer: overall budget plan ===");
  const { agentInput: mbInput } = prepareMediaBuyerInvocation({
    ...
    taskDescription: `Спланировать общий медиа-бюджет (Google Ads + Meta Ads) для потенциального клиента. Стратегия: ${ceoOut.result.strategySummary}. Ресёрч: ${findingsSummary}`,
    clientFactKeys: [],
    complexity: "standard",
    invokeTool: guardedInvoker,
  });
  ...
  const { agentInput: ppcInput } = preparePpcInvocation({
    ...
    taskDescription: `Распределить бюджет между Google Ads и Meta Ads (без запуска/создания кампаний — только план). Общий бюджет: ${mbOut.result.totalBudget}.`,
    clientFactKeys: [],
  ...
  const { agentInput: copyInput } = prepareCopywriterInvocation({
    ...
    taskDescription: "Предложить 3-5 вариантов рекламных заголовков/сообщений для Google Ads и Meta Ads на основе ресёрча и стратегии.",
    clientFactKeys: [],
  ...
  const { agentInput: qaInput } = prepareQaInvocation({
    ...
    taskDescription: "Проверить целостность и адекватность стратегии, медиаплана и текстов перед отправкой владельцу.",
    artifactToReview,
  ...
  const { agentInput: reportInput } = prepareReportGeneratorInvocation({
    ...
    taskDescription: "Собрать итоговое резюме для владельца агентства по результатам пилотного прогона.",
    // Was `[]` — Report Generator had nothing to read and fabricated a
    // content-free narrative with no error (2026-08-27 pilot run). These
    // keys match what each role actually wrote (task-level writes, now
    // archived into project memory above by taskId:key; project-level
    // writes stay bare).
    materialRefs: [
  ...
  writeFileSync(
    new URL("../../../belseltur-pilot-run-output.json", import.meta.url),
    JSON.stringify(fullOutput, null, 2),
  );
  console.log("\n=== DONE. Full output + comm log written to belseltur-pilot-run-output.json ===");
}
