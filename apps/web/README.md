...
- Two tools are real: `site-reader` (`lib/tools/site-reader.ts`, plain HTTP fetch, no credentials) and `web-search` (`lib/tools/web-search.ts`, Perplexity `sonar`, needs `PERPLEXITY_API_KEY` in `.env.local`), dispatched via `lib/real-tools.ts` and used only by Research Agent. Every other tool (`seo-service`, `google-ads`, `vk-ads`, `google-analytics`, `design-tool`, `deployment-tool`) is still a placeholder, so downstream roles still get thin real context about the actual client — expect generic output there until those are wired in too.
