...
import type { AgencyResearcherTaskPayload } from "./agency-researcher-agent.ts";

const MEMORY_LEVELS = ["task", "project"] as const;
const TOOL_IDS = ["site-reader", "web-search", "prospect-store"] as const;

export interface PrepareAgencyResearcherInvocationInput {
...
export function prepareAgencyResearcherInvocation(input: PrepareAgencyResearcherInvocationInput) {
  return prepareAgentInvocation<AgencyResearcherTaskPayload>({
    store: input.store,
