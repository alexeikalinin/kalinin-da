...
import type { OutreachCopywriterTaskPayload } from "./outreach-copywriter-agent.ts";

const MEMORY_LEVELS = ["task", "project"] as const;
const TOOL_IDS = ["prospect-store"] as const;

export interface PrepareOutreachCopywriterInvocationInput {
...
export function prepareOutreachCopywriterInvocation(input: PrepareOutreachCopywriterInvocationInput) {
  return prepareAgentInvocation<OutreachCopywriterTaskPayload>({
    store: input.store,
