...
import type { UxTaskPayload } from "./ux-agent.ts";

const MEMORY_LEVELS = ["task", "project", "client_kb"] as const;

export interface PrepareUxInvocationInput {
...
export function prepareUxInvocation(input: PrepareUxInvocationInput) {
  return prepareAgentInvocation<UxTaskPayload>({
    store: input.store,
