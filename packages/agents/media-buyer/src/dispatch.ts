...
import type { MediaBuyerTaskPayload } from "./media-buyer-agent.ts";

const MEMORY_LEVELS = ["task", "project", "client_kb"] as const;

export interface PrepareMediaBuyerInvocationInput {
...
export function prepareMediaBuyerInvocation(input: PrepareMediaBuyerInvocationInput) {
  return prepareAgentInvocation<MediaBuyerTaskPayload>({
    store: input.store,
    ...
    buildPayload: (prompt, modelId) => ({ prompt, modelId }),
  });
}
