...
import type { LeadScorerTaskPayload } from "./lead-scorer-agent.ts";

const MEMORY_LEVELS = ["task", "project"] as const;
const TOOL_IDS = ["prospect-store"] as const;

export interface PrepareLeadScorerInvocationInput {
...
export function prepareLeadScorerInvocation(input: PrepareLeadScorerInvocationInput) {
  return prepareAgentInvocation<LeadScorerTaskPayload>({
    store: input.store,
    ...
    buildPayload: (prompt, modelId) => ({ prompt, modelId, prospectAgencyId: input.prospectAgencyId }),
  });
}
