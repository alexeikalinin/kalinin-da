...
- **Биддинг**: CPM (Reach objective, оплата за 1000 показов) и CPC (Clicks objective, оплата за
  валидный клик).
- **Гео**: подтверждённый список стран показа рекламы (Help Center, "Ads in ChatGPT: The Basics") — US,
  Canada, Australia, New Zealand, UK, Japan, South Korea, Mexico, Brazil. Список расширяется поэтапно,
  проверять актуальность в Ads Manager на момент подключения конкретного клиента (**Беларусь/Россия
  сейчас в список не входят** — прямая релевантность для существующих клиентов StarMedia/Медавеню/DA.by
  сегодня нулевая).
- **Кому показывается реклама** (потребительская сторона, важно понимать помимо рекламного кабинета):
  только пользователям **Free и Go** планов, залогиненным, в перечисленных выше странах. Plus/Pro/
  Business/Enterprise/Edu — без рекламы. Не показывается пользователям младше 18 (включая тех, кого
  система определила как вероятно младше 18). Реклама НЕ появляется в Temporary Chats, у разлогиненных
  пользователей, после генерации изображений, в ChatGPT Atlas.
- **Формат объявления**: карточка под ответом ChatGPT, отдельно и явно помечена "Sponsored"/"Спонсировано"
  — намеренное визуальное отделение от ответа модели (сохранение доверия к самому ответу). Поля: brand
  name, логотип, headline (до 30 символов), description (до 60 символов), изображение 1:1 (256×256px),
  ссылка. Больше похоже на нативную рекламу, чем на классический баннер.
- **Таргетинг/релевантность**: `context_hints` (advertiser-provided, поле в `/ad_groups`, см. Phase 2
  ниже) — это НЕ ключевые слова в духе Google/Yandex Search: они не гарантируют показ, а только
  подсказывают модели релевантности тему/ключевые понятия, где продукт уместен. Реальный подбор объявления
  делает модель релевантности OpenAI на основе контекста и интента текущего разговора + `context_hints`
  + гео-таргетинг + (если персонализация включена) сигналы из истории использования ChatGPT.
- **Приватность/персонализация**: пользователь может включить/выключить ads personalization (не убирает
  рекламу, только снижает её релевантность), видеть историю показанных тем, удалять данные показов одним
  тапом. **Рекламодателям никогда не передаются сами чаты, memory, персональные данные или IP пользователя**
  — важно знать это, объясняя клиенту, что таргетинг работает на уровне тематики разговора, а не на
  профиле конкретного человека, в отличие от behavioral-таргетинга Meta Ads.
