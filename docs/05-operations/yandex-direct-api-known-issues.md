...
## 10. Нет `adgroups.suspend`/`resume` — "приостановить группу" не существует
как отдельная операция, найдено 2026-09-23 (каннибализация 708468419 vs
713471948, родинки)

**Симптом:** вызов `adgroups.suspend` с `SelectionCriteria.Ids` возвращает
HTTP 200, но с top-level `error_code: 55, error_string: "Операция не
найдена"` — в отличие от `campaigns.suspend`/`ads.suspend`, у которых
операция суспенда реально существует, у `adgroups` в API v5 её просто нет
в принципе (не гоча формата запроса — сама операция отсутствует в
сервисе).

**Фикс/обходной путь:** «приостановить группу» = приостановить ВСЕ
объявления группы (`ads.suspend`, уже есть как `setAdStatus`) + все
РУЧНЫЕ ключевые фразы группы (`keywords.suspend`) — этого достаточно,
показов у группы не будет. Автотаргетинг-псевдоключ (`Keyword:
"---autotargeting"`) `keywords.suspend` отдельно не останавливает
(`error 8305 "Автотаргетинг не может быть остановлен"`), но это не
проблема: без единого активного объявления в группе автотаргетинг
показывать нечего, он не работает "в обход" отсутствия объявлений.
Живая проверка 2026-09-23: у группы 5736114240 оба объявления давно были
в `State: SUSPENDED`, а Reports API по этой группе не показал ни одного
показа с 2026-08-12 — то есть группа уже была де-факто выключена именно
через этот путь, задолго до формального запроса на «приостановку».

Реализовано как `suspendAdGroupServing(adGroupId, clientLogin?,
accessTokenEnv?)` в `yandex-direct.ts` — читает состояние
объявлений/ключей группы, приостанавливает только то, что ещё не
приостановлено, пропускает автотаргетинг-строку, возвращает, что именно
было тронуто. Не путать с несуществующим `setAdGroupStatus` — такой
функции в этом файле больше нет, использовать `suspendAdGroupServing`.

**Смежная находка**: `campaigns.update`'s `NegativeKeywords.Items` молча
дедуплицирует однокоренные формы — отправка одновременно `"родинки"` и
`"родинка"`/`"родинку"` в одном вызове привела к тому, что
`totalNegativeCount` в ответе вырос на все 5 присланных фраз, но при
последующем чтении (`campaigns.get`) в списке реально осталось только 3
уникальные строки (`"родинки"`, `"удаление родинки"`, `"удалить
родинку"`) — единственное число `"родинка"`/`"родинку"` пропало, судя по
всему как покрытое той же леммой `"родинки"`. Самопроверка после любой
записи минус-фраз должна сверять реально СЧИТАННЫЙ список, а не
доверять `totalNegativeCount`, возвращённому самим write-вызовом.
